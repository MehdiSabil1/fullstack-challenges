...
1. **首先是数据层** — 员工模型和 repository，然后是订单模型和 repository。使用 `rake` 验证每个部分。
2. **其次是功能** — 每次构建一个功能，并通过运行应用程序来测试它。

我们还将添加一个登录功能，它与其他功能有些不同，因为你必须先登录才能做任何其他事情。
...
编写一些代码来实现这一点，并对模型进行崩溃测试。然后通过运行 `rake order` 来测试代码。
...
让我们来制作 `OrdersController`。下面是我们要实现的 **用户操作**：
...
⚠️ **分开编写代码（Code in silo）** — 每次实现一个操作，并在继续下一个之前通过运行应用程序来测试它。
